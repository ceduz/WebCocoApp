...
#def calculate_evaporation(df, scenario_precip_column, eto):
    #"""Calculate evaporation based on scenario-specific precipitation and ETO."""
    #return [(h * 0.2 if p > 0 and p >= h * 0.2 else p) for p, h in zip(df[scenario_precip_column], eto)]
...
def two_stage_model(df, merg_sce_colum_prect, merg_sce_probs_prect, escen_prectotcorr):
    ...
    for s in merg_sce_colum_prect:
        ...
        evaporated = {}
        #scenario identifiers and 'ETO' is a predefined list of evapotranspiration values
        ...
        I_t0 = {s: LpVariable(f"Irrigation_t0_{s}", lowBound=0, cat='Continuous')}
        D_t0 = {s: LpVariable(f"Draining_t0_{s}", lowBound=0, cat='Continuous')}
        W_t = {s: LpVariable(f"W_t_{s}", lowBound=critical_depletion, cat='Continuous')}
        DR_t0 = {s: LpVariable(f"Depletion_t_{s}", lowBound=0, cat='Continuous')}
        # Auxiliar variables
        x_minus0 = {s: LpVariable(f"xminus_t0_{s}", lowBound=0, cat='Continuous')}
        x_plus0 = {s: LpVariable(f"xplus_t0_{s}", lowBound=0, cat='Continuous')}
        y_minus0 = {s: LpVariable(f"yminus_t0_{s}", lowBound=0, cat='Continuous')}
        y_plus0 = {s: LpVariable(f"yplus_t0_{s}", lowBound=0, cat='Continuous')}
        f_water0 = {s: LpVariable(f"fwater_t0_{s}", lowBound=0, cat='Continuous')}
        ...
        I_t15 = {s: LpVariable(f'Irrigation_t15_{s}', lowBound=0, cat='Continuous')}
        D_t15 = {s: LpVariable(f'Draining_t15_{s}', lowBound=0, cat='Continuous')}
        W_t15 = {s: LpVariable(f"W_t15_{s}", lowBound=critical_depletion, upBound=critical_water, cat='Continuous')}
        DR_t15 = {s: LpVariable(f"Depletion_t15_{s}", lowBound=0, cat='Continuous')}
        # Auxiliar variables
        x_minus15 = {s: LpVariable(f"xminus_t15_{s}", lowBound=0, cat='Continuous')}
        x_plus15 = {s: LpVariable(f"xplus_t15_{s}", lowBound=0, cat='Continuous')}
        y_minus15 = {s: LpVariable(f"yminus_t15_{s}", lowBound=0, cat='Continuous')}
        y_plus15 = {s: LpVariable(f"yplus_t15_{s}", lowBound=0, cat='Continuous')}
        f_water15 = {s: LpVariable(f"fwater_t15_{s}", lowBound=0, cat='Continuous')}
        ...
        objective_contributions_t1.append(merg_sce_probs_prect[inx_probabil] * contribution_t1)

        # Calculate contributions from stage 2
        objective_contributions_t15 = []
        
        contribution_t15 = merg_sce_probs_prect[inx_probabil] * CCh * ((f_water15[s]*df['EF_t'].iloc[14]) - (x_minus15[s]*df['EF_t'].iloc[14]))
        ...
        for s2 in merg_sce_colum_prect:
            if s != s2:
                model += I_t0[s] == I_t0[s2], f"Non_Anticipativity_I_t0_{s}_{s2}"
                model += D_t0[s] == D_t0[s2], f"Non_Anticipativity_D_t0_{s}_{s2}"
    ...
    for s in scenarios:
        precip_column = f"P15_{s}"  # Adjust this if your column naming convention is different
        evaporated[s] = calculate_evaporation(df, precip_column, ETO)
    ...
    # Stage 1 Decision Variables (scenario-independent)
    # Define scenario-specific irrigation and drainage variables at the initial stage
    I_t0 = {s: LpVariable(f"Irrigation_t0_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    D_t0 = {s: LpVariable(f"Draining_t0_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    W_t = {s: LpVariable(f"W_t_{s}", lowBound=critical_depletion, cat='Continuous') for s in scenarios}
    DR_t0 = {s: LpVariable(f"Depletion_t_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    # Auxiliar variables
    x_minus0 = {s: LpVariable(f"xminus_t0_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    x_plus0 = {s: LpVariable(f"xplus_t0_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    y_minus0 = {s: LpVariable(f"yminus_t0_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    y_plus0 = {s: LpVariable(f"yplus_t0_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    f_water0 = {s: LpVariable(f"fwater_t0_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    ...
    I_t15 = {s: LpVariable(f'Irrigation_t15_{s}', lowBound=0, cat='Continuous') for s in scenarios}
    D_t15 = {s: LpVariable(f'Draining_t15_{s}', lowBound=0, cat='Continuous') for s in scenarios}
    W_t15 = {s: LpVariable(f"W_t15_{s}", lowBound=critical_depletion, upBound=critical_water, cat='Continuous') for s in scenarios}
    DR_t15 = {s: LpVariable(f"Depletion_t15_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    # Auxiliar variables
    x_minus15 = {s: LpVariable(f"xminus_t15_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    x_plus15 = {s: LpVariable(f"xplus_t15_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    y_minus15 = {s: LpVariable(f"yminus_t15_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    y_plus15 = {s: LpVariable(f"yplus_t15_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    f_water15 = {s: LpVariable(f"fwater_t15_{s}", lowBound=0, cat='Continuous') for s in scenarios}
    ...
    for s in scenarios:
        contribution_t1 = CCh * ((f_water0[s]*df['EF_t'][0]) - (x_minus0[s]*df['EF_t'][0]) )
        objective_contributions_t1.append(probabilities[s] * contribution_t1)

    # Calculate contributions from stage 2
    objective_contributions_t15 = []
    for s in scenarios:
        contribution_t15 = probabilities[s] * CCh * ((f_water15[s]*df['EF_t'].iloc[14]) - (x_minus15[s]*df['EF_t'].iloc[14]))
        objective_contributions_t15.append(contribution_t15)
    ...
    # Base scenario constraints at time t with scenario-specific irrigation and drainage
    for s in scenarios:
        model += ((ETC[0] + D_t0[s] + evaporated[s][0]) - (df[f"P15_{s}"].iloc[0] + I_t0[s] + DR_0)) == DR_t0[s], f"Depletion_Calculation_t_{s}"
        model += (W_0 + df[f"P15_{s}"].iloc[0] + I_t0[s]) - (D_t0[s] + ETC[0] + evaporated[s][0] + DR_0) <= W_t[s], f"Water_Balance_at_t1_Scenario_{s}"
    ...
    for s in scenarios:
        ...
        for t in range(2, t_final):  # Loop from t=2 to t_final (inclusive)
            ...
            if t == 2:
                # Use W_t[s] as the lagged value for t=2 (assuming W_t[s] is defined for t=1)
                model += (ETC[t-1] + evaporated[s][t-1]+current_RO_t[(s, t)]) - (df[f"P15_{s}"].iloc[t-1] + DR_t0[s]) == current_DR_t[(s, t)], f"Depletion_Calculation_at_t{t}_Scenario_{s}"
                model += (W_t[s] + df[f"P15_{s}"].iloc[t-1]) - (ETC[t-1] + DR_t0[s]+current_RO_t[(s, t)]+evaporated[s][t-1]) == current_W_t[(s, t)], f"Water_Balance_at_t{t}_Scenario_{s}"

                # model += critical_water-current_DR_t[(s, t)] <= current_W_t[(s, t)], f"PAW_t{s}_{t}" #New constraint

            else:
                # Use the previous time step's water level for t > 2
                model += (ETC[t-1] + evaporated[s][t-1]+current_RO_t[(s, t)]) - (df[f"P15_{s}"].iloc[t-1] + current_DR_t[(s, t-1)]) == current_DR_t[(s, t)], f"Depletion_Calculation_at_t{t}_Scenario_{s}"
                model += (current_W_t[(s, t-1)] + df[f"P15_{s}"].iloc[t-1]) - (ETC[t-1]+ current_DR_t[(s, t-1)]+current_RO_t[(s, t)]+evaporated[s][t-1]) == current_W_t[(s, t)], f"Water_Balance_at_t{t}_Scenario_{s}"
    ...
    # Base scenario constraints at time t=15  with scenario-specific irrigation and drainage
    for s in scenarios:
        model += (ETC[14] + evaporated[s][14]+D_t15[s]) - (df[f"P15_{s}"].iloc[14] +I_t15[s]+ current_DR_t[(s, 14)]) == DR_t15[s], f"Depletion_Calculation_at_t15_Scenario_{s}"
        model += (current_W_t[(s, 14)] + df[f"P15_{s}"].iloc[14] + I_t15[s]) - (D_t15[s] +ETC[14]+evaporated[s][t-1] + current_DR_t[(s, 14)]) == W_t15[s], f"Water_Balance_at_t15_Scenario_{s}"
        model += critical_depletion <= W_t15[s] <= critical_water
        # model += critical_water-DR_t15[s] <= W_t15[s], f"PAW_t15{s}" #New constraint
    ...
    print("Show balance for initial t")
    for s in scenarios:
        precipitation = df[f"P15_{s}"].iloc[0]
        print(f'Irrigation: {I_t0[s].varValue} Drainage: {D_t0[s].varValue}')
        print(f"DR for Scenario {s}: {ETC[0]}+{D_t0[s].varValue}+ {evaporated[s][0]}- {precipitation} - {I_t0[s].varValue} - {DR_0.varValue} == {DR_t0[s].varValue}")
        print(f"DR for Scenario {s}: {ETC[0]+D_t0[s].varValue + evaporated[s][0] - precipitation - I_t0[s].varValue + DR_0.varValue} == {DR_t0[s].varValue}")
        print(f"W for Scenario {s}: {W_0.value()} + {precipitation} + {I_t0[s].varValue} - {D_t0[s].varValue} - {ETC[0]} - {DR_0.varValue} <= {W_t[s].varValue}")
    ...
    # Debugging: Show balance for t = 15
    for s in scenarios:
        precipitation = df[f"P15_{s}"].iloc[14]
        print(f'Scenario: {s}')
        print(f'Irrigation at t=15: {I_t15[s].varValue}, Drainage at t=15: {D_t15[s].varValue}')
        print(f"DR for Scenario {s} at t=15: {ETC[14]} + {evaporated[s][14]} + {D_t15[s].varValue} - {precipitation} - {I_t15[s].varValue} - {current_DR_t[(s, 14)].varValue} == {DR_t15[s].varValue}")
        print(f"DR for Scenario {s} at t=15 (Calculated): {ETC[14] + evaporated[s][14] + D_t15[s].varValue - precipitation - I_t15[s].varValue - current_DR_t[(s, 14)].varValue} == {DR_t15[s].varValue}")
    ...
    for s in scenarios:
        for t in range(2, t_final):
            precipitation = df[f"P15_{s}"].iloc[t-1]
            print(f'Scenario: {s}, Time step: {t}')
            print(f'Precipitation at t={t}: {precipitation}')
            print(f"Final available water at t={t}: {current_W_t[(s, t)].varValue}")
            print(f"DR for Scenario {s} at t={t}: {ETC[t-1]} + {evaporated[s][t-1]} + {current_RO_t[(s, t)].varValue} - {precipitation} - {DR_t0[s].varValue if t == 2 else current_DR_t[(s, t-1)].varValue} == {current_DR_t[(s, t)].varValue}")
...
def main(pkFinca, fore_sce_dt_prect, merg_sce_colum_prect, merg_sce_probs_prect):
    df = import_data_clim(pkFinca, 179)
    df = df.head(15)

    escen_prectotcorr = result_scen(merg_sce_colum_prect, fore_sce_dt_prect)

    result = two_stage_model(df, merg_sce_colum_prect, merg_sce_probs_prect, escen_prectotcorr)